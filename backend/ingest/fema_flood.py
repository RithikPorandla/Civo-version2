"""Ingest FEMA NFHL flood hazard polygons clipped to MA town(s).

Source:
    https://arcgisserver.digital.mass.gov/arcgisserver/rest/services/FEMA/FEMA_National_Flood_Hazard_Layer/FeatureServer/0
"""
...
URL = (
    "https://arcgisserver.digital.mass.gov/arcgisserver/rest/services/FEMA/"
...
INSERT INTO flood_zones (
    fld_zone, zone_subty, sfha_tf, static_bfe, dfirm_id, attrs, geom
) VALUES (
    :fld_zone, :zone_subty, :sfha_tf, :static_bfe, :dfirm_id, CAST(:attrs AS jsonb),
...
def ingest_town(town: str) -> int:
    ...
    with httpx.Client() as client, engine.begin() as conn:
        ...
        if n_del:
            print(f"  [{town}/flood] deleted {n_del}")
        ...
        if batch:
            conn.execute(INSERT_SQL, batch)
            total += len(batch)
    print(f"  [{town}/flood] +{total}")
    return total


def main() -> None:
    ap = argparse.ArgumentParser(description="Ingest FEMA NFHL for MA towns.")
    ap.add_argument("--town", action="append")
    args = ap.parse_args()
    towns = args.town or TARGET_TOWNS
    grand = sum(ingest_town(t) for t in towns)
    print(f"Done. {grand} flood-zone rows across {len(towns)} town(s).")
