...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("shape_index", sa.Float),        # 4π*area/perimeter² → 1=circle, <1=irregular
        ...
        sa.Column("aerial_veg_index", sa.Float),     # red-blue ratio proxy
        sa.Column("aerial_edge_density", sa.Float),  # structure density
    ...
    op.create_index("ix_parcel_ml_features_extracted", "parcel_ml_features", ["extracted_at"])
    ...
    op.create_table(
        ...
        sa.Column("support_score", sa.Float),            # positive signals
        sa.Column("opposition_score", sa.Float),         # negative signals
        sa.Column("document_count", sa.Integer, server_default="0"),
        sa.Column("key_concerns", JSONB),                # list of concern strings
        sa.Column("key_support", JSONB),                 # list of support strings
        sa.Column("sources", JSONB),                     # list of source URLs/dockets
